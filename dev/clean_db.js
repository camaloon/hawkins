...
//// Cleaning routine /////////////////////////////////////////////////////////
//
// Remove builds (including its logs and examples) older than `days_to_keep`
// days.
//
///////////////////////////////////////////////////////////////////////////////
...
to_delete_ref.once("value", function(snapshot) { // snapshot: Hash of buildHAsh: buildObject(startedAt, finishedAt, status, push, worker)
  console.log("Deleting...")
  Object.keys(snapshot.val()).forEach(function(buildHash) {
    console.log("Deleting ref: " + buildHash);
    Promise.all([
      db.ref("logs/" + buildHash).remove(),
      db.ref("examples/" + buildHash).remove(),
      db.ref("builds/" + buildHash).remove(),
    ])
    .then(function(){ console.log("Deleted ref: " + buildHash)})
    .catch(console.log.bind(this, "Errorll!!"));
  });
